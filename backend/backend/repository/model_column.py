...
class ModelColumn(SQLModel, table=True):  # type: ignore
    
    __tablename__ = "model_column"  # type: ignore
    
    
    model_id: Mapped[str] = mapped_column(ForeignKey(f"{Model.__tablename__}.id"))
    
    is_calculated: Mapped[bool] = mapped_column(nullable=False)  # Is calculated field
    
    display_name: Mapped[str] = mapped_column(nullable=False)  # Column name displayed in UI
    ...
    custom_expression: Mapped[Optional[str]] = mapped_column(nullable=True, default=None)
